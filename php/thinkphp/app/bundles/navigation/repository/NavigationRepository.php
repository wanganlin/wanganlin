...
class NavigationRepository extends CurdRepository implements RepositoryInterface
{
     ...
     * 添加
     */
    public function createByEntity(NavigationEntity $entity): int
    {
     ...
     * 按照ID查询返回对象
     */
    public function findOneEntityById(int $id): ?NavigationEntity
    {
     ...
     * 按照条件查询
     */
    public function findOneEntity(array $condition): ?NavigationEntity
    {
     ...
     * 定义数据数据模型类
     */
    public function model(string $modelName = 'Navigation'): Model
    {
        $model = '\\app\\bundles\\navigation\\model\\'.$modelName.'Model';
