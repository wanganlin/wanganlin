...
namespace app\bundles\category\repository;
...
class CategoryRepository extends CurdRepository implements RepositoryInterface
{
     ...
     * 添加
     */
    public function createByEntity(CategoryEntity $entity): int
    {
     ...
     * 按照ID查询返回对象
     */
    public function findOneEntityById(int $id): ?CategoryEntity
    {
     ...
     * 按照条件查询
     */
    public function findOneEntity(array $condition): ?CategoryEntity
    {
     ...
     * 定义数据数据模型类
     */
    public function model(string $modelName = 'Category'): Model
    {
        $model = '\\app\\bundles\\category\\model\\'.$modelName.'Model';
